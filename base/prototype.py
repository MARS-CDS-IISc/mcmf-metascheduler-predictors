...
class TriggerMetaScheduler(JobEvent):pass
...
class Job(object):
    def __init__(self, id, user_estimated_run_time, actual_run_time, num_required_processors, origin_site_id, \
        ...
        self.sitemap = None
	self.isMeta = False
        self.original_ert = user_estimated_run_time
        self.original_run = actual_run_time
        ...
        self.actual_electricity_price = 0
        self.wait_time_prediction = 0
        self.response_time_prediction = 0
        ...
        # not used by base
        self.original_submit_time = submit_time
    ...
    def get_watts(self, power_per_core):
        return (power_per_core*self.num_required_processors)
    ...
    def __repr__(self):
        return type(self).__name__ + "<id=%(id)s, user_estimated_run_time=%(user_estimated_run_time)s, actual_run_time=%(actual_run_time)s, required_processors=%(num_required_processors)s, start_to_run_at_time=%(start_to_run_at_time)s, submit_time=%(submit_time)s, user_id=%(user_id)s, actual_dollars=%(actual_electricity_price)s>" % vars(self)
...
class Machine(object):
    ...
    def start_job_handler(self, event):
        ...
        if event.job.start_to_run_at_time not in (-1, event.timestamp):
            ...
            return
        #print 'SJH called for event', event
    ...
    def _add_job(self, job, current_timestamp):
        ...
        #self.event_queue.RunningJobs.append(job)
        #self.event_queue.QueuedJobs.remove(job)
        #queuedJobIDs = [j.id for j in self.event_queue.QueuedJobs]
        #runningJobIDs = [j.id for j in self.event_queue.RunningJobs]
        #print 'Job', job.id, 'Site:', job.sitemap.id
        #print 'Once here at', current_timestamp
        #print job.sitemap.queuedJobs
        job.sitemap.queuedJobs.remove(job)
        job.sitemap.runningJobs.append(job)

        if job.sitemap.wait_predictor != None:
            job.sitemap.wait_predictor.notify_job_start_event(job, job.sitemap.queuedJobs, job.sitemap.runningJobs)
...
class ValidatingMachine(Machine):
    ...
    def __init__(self, num_processors, event_queue):
        ...
        #print 'Creating new machine with EQ', event_queue
        self.jobs = set()
...
def _job_input_to_job(job_input, total_num_processors):
    ...
    if job_input.run_time <= 0 or job_input.num_allocated_processors <= 0 or job_input.submit_time < 0 :
        return Job(
            ...
            origin_site_id = job_input.original_submit_site,
        )
...
class Site:
    def __init__(self, id, loc, name, tz_lag, num_procs, scheduler, ppc, gflops, max_ert, max_req):
        self.id = id
        self.location = loc
        self.timezone_lag = tz_lag
        self.num_processors = num_procs
        self.scheduler = scheduler
        self.queueSimulator = None
        self.price_predictor = None
        self.wait_predictor = None
        self.queuedJobs = []
        self.runningJobs = []
        self.machine = None
        self.power_per_core = ppc
        self.name = name
        self.gflops = gflops
        self.max_resp_in_cycle = -float('inf')
        self.max_ert = max_ert
